...
#Calling the lunar phase function if the day and month given are found.
    def get(self, index, day):
        try:
            Phases = ["New Moon", "Waning Crescent", "First Quarter", "Waxing Gibbous", "Full Moon", "Wanning Gibbous", "Third Quarter", "Waning Crescent", "New Moon"]
            cur_idx = 0
            cur_node = self.head
            while True:
                cur_node = cur_node.next
                if index == cur_node.name:
                    ...
                    message = "On, " + cur_node.name + cur_node.listDay[day - 1]  + " The Moon phase was: " + calculate_lunar_phase(jdn)
                    return message
                cur_idx += 1
        except:
